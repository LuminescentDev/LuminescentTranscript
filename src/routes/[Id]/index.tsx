...
export default component$(() => {
  ...
  useVisibleTask$(() => {
    ...
    console.log(`Timezone changed to ${store.timezone}`)
  });
      ...
      <header>
        <nav class="z-10 fixed top-0 w-screen bg-discord-600/50 backdrop-blur-2xl border-b border-b-discord-900">
          <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="relative flex h-12 items-center">
              ...
              <div class="flex flex-1 items-center justify-start">
                ...
                <span class="ml-3 font-semibold text-gray-50">
                  {channel.name}
                </span>
              </div>
              
              <div class="flex items-center justify-center text-gray-50">
                  <div class="hidden sm:flex space-x-4 font-semibold">
          ...
          <div class="hidden px-6 py-2 mx-auto max-w-2xl" id="mobile-menu">
            <div class="flex flex-col gap-2 my-3 py-4 px-6 justify-center items-center bg-discord-900 rounded-lg">
              {guild.name &&
                <div class="flex sm:hidden space-x-4 text-gray-50 font-semibold">
                  {guild.icon && <img width={24} height={24} class="h-6 w-6 mr-3 rounded-full" src={guild.icon} alt="Server Icon" />}
                  {guild.name}
                </div>
              }
              <p class="text-sm">
                Created on {new Date(time).toLocaleString('default', { timeZone: store.timezone, weekday: 'long', month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true })}
              </p>
            </div>
          </div>
        </nav>
          ...
          return <>
            ...
            <div class="group">
              ...
              <div class="relative hidden group-hover:block">
                <div class="absolute flex -top-14 right-6 bg-discord-600 border border-discord-800 hover:shadow-md rounded-md">
                  <button class="hover:bg-discord-400 p-1.5 cursor-pointer rounded-l-md" onClick$={() => {
                  ...
                  <button class="hover:bg-discord-400 p-1.5 cursor-pointer" onClick$={() => {
                  ...
                  <a href={`https://discord.com/channels/${guild.id}/${channel.id}/${log.id}`} class="hover:bg-discord-400 p-1.5 rounded-r-md cursor-pointer">
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
      ...
      meta: [
          {
              name: 'description',
              content: `${logs.length} Messages - Created on ${new Date(time).toLocaleString('default', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true, timeZoneName: 'short' })}`
          },
          {
